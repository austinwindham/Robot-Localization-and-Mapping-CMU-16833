...
def create_linear_system(x, odoms, observations, sigma_odom, sigma_observation,
    ...
    J_odo = sqrt_inv_odom @ np.array([[-1, 0, 1, 0],
                                      [0, -1, 0, 1]])

    for i in range(n_odom):

        A[2*(i + 1):2*(i + 2), 2*i:2*(i + 2)] = J_odo
    ...
    for i in range(n_obs):
        ...
        obs = bearing_range_estimation(x, pose_idx, landmark_idx, n_poses)

        d_obs = np.array([warp2pi(measurement[0] - obs[0]), 
                        measurement[1] - obs[1]])

        A_l = sqrt_inv_obs @ compute_meas_obs_jacobian(x, pose_idx, landmark_idx,n_poses)

        A[2*(n_odom + 1 + i):2*(n_odom + 2 + i), 2*pose_idx:2*(pose_idx + 1)] = A_l[:, 0:2]

        A[2*(n_odom + 1 + i):2*(n_odom + 2 + i), 2*(n_poses + landmark_idx): 2*(n_poses + landmark_idx + 1)] = A_l[:, 2::]

        b[2*(n_odom + 1 + i):2*(n_odom + 2 + i)] = sqrt_inv_obs @ d_obs
