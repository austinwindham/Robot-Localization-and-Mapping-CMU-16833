...
def create_linear_system(odoms, observations, sigma_odom, sigma_observation,
    ...
    J_odo = np.array(([[-1, 0, 1, 0],
                       [0, -1, 0, 1]
                       ]))
    
    A_odo = sqrt_inv_odom @ J_odo

    for i in range(n_odom):

        A[2*(i+1):2*(i+2), 2*i:2*(i+2)] = A_odo
        b[2*(i+1):2*(i+2)] = sqrt_inv_odom @ odoms[i]

    # TODO: Then fill in landmark measurements
    J_obs = np.array(([[-1, 0, 1, 0],
                       [0, -1, 0, 1]
                       ]))
    ...
    for i in range(n_obs):
        ...
        # make A
        A[2*(n_odom + 1 + i):2*(n_odom +2 + i), 2*pose_idx:2*(pose_idx + 1)] = A_obs[:, 0:2]
        A[2*(n_odom + 1 + i):2*(n_odom +2 + i), 2*(n_poses+landmark_idx):2*(n_poses+landmark_idx +1)] = A_obs[:, 2::]
