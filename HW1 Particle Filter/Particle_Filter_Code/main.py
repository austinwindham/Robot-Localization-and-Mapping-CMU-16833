...
def visualize_timestep(X_bar, tstep, output_path):
    x_locs = X_bar[:, 0] / 10.0
    y_locs = X_bar[:, 1] / 10.0
    scat = plt.scatter(x_locs, y_locs, c='r', marker='o', s=15)
...
def init_particles_freespace(num_particles, occupancy_map):
    ...
    # initialize [x, y, theta] positions in world_frame for all particles
    X_bar_init = np.ones((num_particles, 4), np.float64)
    ...
    while num_particles > 0:
        ...
        # put particles in freespace
        if occupancy_map[j][i] == 0:
            X_bar_init[num_particles-1] = np.hstack((x, y, theta, w))
            num_particles -= 1
...
if __name__ == '__main__':
    ...
    for time_idx, line in enumerate(logfile):
        ...
        if args.visualize and time_idx:
            visualize_timestep(X_bar, time_idx, args.output)
